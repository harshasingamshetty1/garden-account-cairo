...
from e2e.utils.typed_data import (
    OutsideExecution,
    get_test_call,
    CalldataValidation,
    AllowedMethod,
...
async def test_session_execution(
    ...
    (
        ...
        destination_account,
    ) = await setup_session_account_env(
        is_gas_sponsored_execution,
        second_signer_type,
        multisig_threshold,
    ...
    if not is_v3 and not is_gas_sponsored_execution:
        with pytest.raises(
            (TransactionRevertedError, ClientError),
    ...
    assert (
        txn_receipt_contains_event(
            receipt,
            [get_selector_from_name(event_name), oe.get_hash(session_account.address)],
            [
                session_owner_identifier,
                oe.execute_after,
                oe.execute_before,
        ...
        is True
    ), "no execute session started event"
        ...
        calls=[
            AllowedMethod(
                ...
                calldata_validations=[
                    ...
                    # spender
                    CalldataValidation(offset=0, value=destination_account.address),
                ],
            ),
            AllowedMethod(
                to_addr=STRK_ADDRESS,
    ...
    oe = session_request_builder(
        ...
        calls=[
            ...
            AllowedMethod(
                ...
                selector=get_selector_from_name("approve"),
                calldata_validations=[
                    CalldataValidation(offset=0, value=devnet_account.address)
                ],
            ),
            AllowedMethod(
                to_addr=STRK_ADDRESS,
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "added_signer_type", [SECP256R1_SIGNER_TYPE, WEBAUTHN_SIGNER_TYPE]
)
@pytest.mark.parametrize(
...
async def test_session_cache_invalidation_when_signer_added(
    ...
    token_spent = await get_spending_limit_amount_spent(
        session_account, oe.get_hash(session_account.address), FEE_CONTRACT_ADDRESS
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "existing_signer_type", [SECP256R1_SIGNER_TYPE, WEBAUTHN_SIGNER_TYPE]
)
@pytest.mark.parametrize(
    ["is_gas_sponsored_execution", "is_v3"],
    [(False, True), (True, False), (True, True)],
...
async def test_session_cache_invalidation_when_signer_removed(
    ...
    calls = Call(
        ...
        calldata=[
            poseidon_hash_many(flatten_seq(session_account.secp256r1_keypair[1])),
            existing_signer_type,
            0,
        ],
    )
...
async def test_session_cache_invalidation_when_multisig_changes(
    ...
    (
        ...
        is_gas_sponsored_execution, SECP256R1_SIGNER_TYPE, multisig_initial_value
    )
...
async def test_session_execution_with_etd_present(
    ...
    requests.post(f"{devnet_url}/increase_time", json={"time": int(24 * 5 * 60 * 60)})
...
@pytest.mark.parametrize("is_v2_typed_data", [False, True])
async def test_session_execution_revoke(
    init_starknet,
    account_deployer,
    setup_session_account_env,
    is_revoked_on_start,
    ...
    revoke_tx = await session_account.execute_v1(
        Call(
            to_addr=session_account.address,
    ...
    assert (
        ...
        is True
    ), "no execute session revoked event"
...
async def test_session_multiple_concurrent(
    ...
    (
        ...
        is_gas_sponsored_execution_first, second_signer_type, multisig_threshold
    )

    block = await devnet_client.get_block()
    block_timestamp = block.timestamp

    oe1 = session_request_builder(
        session_account,
        session_owner_identifier,
        block_timestamp + 3600,
        block_timestamp - 3600,
    ...
    (
        _,
        second_execute_session_call,
        second_session_request_builder,
        second_event_name,
    ...
    assert (
        txn_receipt_contains_event(
            receipt,
            [get_selector_from_name(event_name), oe1.get_hash(session_account.address)],
            [
                session_owner_identifier,
                oe1.execute_after,
                oe1.execute_before,
...
@pytest.mark.parametrize("is_v2_typed_data", [False, True])
async def test_session_execution_expiry(
    init_starknet,
    account_deployer,
    setup_session_account_env,
    second_signer_type,
    ...
    requests.post(f"{devnet_url}/increase_time", json={"time": int(2 * 24 * 60 * 60)})
...
async def test_session_execution_with_invalid_call(
    ...
    with pytest.raises(
        (TransactionRevertedError, ClientError), match=encode_string_as_hex("BAD_CALL")
    ):
        oe_call = oe.prepare_call(
            [
                get_test_call(destination_account.address, 100),
        ...
        if is_gas_sponsored_execution:
            if is_v2_typed_data:
                oe_call.calldata[31] = get_selector_from_name("increase_allowance")
            else:
...
@pytest.mark.parametrize("is_gas_sponsored_execution", [False, True])
@pytest.mark.parametrize("is_v2_typed_data", [False, True])
async def test_session_execution_invalid_sig(
    init_starknet,
    account_deployer,
    setup_session_account_env,
    second_signer_type,
...
async def test_session_gas_limit_overload(
    ...
    (
        ...
        session_owner_identifier,
        destination_account,
    ) = await setup_session_account_env(False, second_signer_type, multisig_threshold)
...
@pytest.mark.parametrize(
    "dai_transfer_from_entrypoint", ["transfer_from", "transferFrom"]
)
@pytest.mark.parametrize("is_v2_typed_data", [False, True])
async def test_session_spending_limits(
    ...
    calls = [
        *[
            ...
            for name in relevant_erc20_entrypoints + relevant_snake_erc20_entrypoints
        ],
        *[
            AllowedMethod(
                to_addr=usdc_token.address,
    ...
    if is_after_cache:
        ...
        spending = await get_spending_limit_amount_spent(
            session_account, oe.get_hash(session_account.address), usdc_token.address
        )
    ...
    for entrypoint in relevant_erc20_entrypoints + relevant_snake_erc20_entrypoints:
        with pytest.raises(
            (ClientError, TransactionRevertedError),
    ...
    with pytest.raises(
        (ClientError, TransactionRevertedError), match="is not deployed"
    ):
        tx = await execute_session_call(
            oe.prepare_call(
                [
...
async def test_session_revoke_via_outside_execution(
    ...
    (
        ...
        session_owner_identifier,
        destination_account,
    ) = await setup_session_account_env(False, None, 0)
...
async def test_session_self_calls_are_blocked(
    ...
    (
        ...
        session_owner_identifier,
        destination_account,
    ) = await setup_session_account_env(is_gas_sponsored_execution, None, 0)
    ...
    with pytest.raises(
        (ClientError, TransactionRevertedError), match=encode_string_as_hex("SELF_CALL")
    ):
        tx = await execute_session_call(
            oe.prepare_call(
                [
...
async def test_session_bad_call_hints(
    ...
    with pytest.raises(
        (ClientError, TransactionRevertedError), match=encode_string_as_hex("BAD_CALL")
    ):
        tx = await execute_session_call(
            oe.prepare_call(
                [
