...
async def test_add_secp256r1_signer(
    ...
    webauthn_secp256r1_pubk = (
        None
        if webauthn_secp256r1_keypair is None
        else flatten_seq(webauthn_secp256r1_keypair[1])
    )
    account, _ = await account_deployer(
        stark_privk, webauthn_secp256r1_pubk, 0, is_webauthn=True
    )
    account: Account
    ...
    webauthn_secp256r1_signer = (
        None
        if webauthn_secp256r1_keypair is None
        else create_webauthn_signer(webauthn_secp256r1_keypair[0])
    )
    ...
    assert (
        ...
        is True
    ), "no secp256r1 signer added event emitted"
    ...
    if multisig_threshold == 0:
        ...
        if webauthn_secp256r1_signer is None:
            account.signer = create_multisig_signer(stark_signer, secp256r1_signer)
            await execute_calls(account, balanceof_call)
        else:
            await assert_execute_fails_with_signer(
                account,
            ...
            stark_hws_multisg = create_multisig_signer(stark_signer, secp256r1_signer)
            await assert_execute_fails_with_signer(
                account,
                balanceof_call,
                stark_hws_multisg,
...
async def test_add_webauthn_signer(
    ...
    hws_secp256r1_pubk = (
        None if hws_secp256r1_keypair is None else flatten_seq(hws_secp256r1_keypair[1])
    )
    ...
    hws_secp256r1_signer = (
        ...
        else create_secp256r1_signer(hws_secp256r1_keypair[0])
    )
    ...
    add_webauthn_call = Call(
        to_addr=account.address,
        selector=get_selector_from_name("add_secp256r1_signer"),
        calldata=[*webauthn_secp256r1_pubk, WEBAUTHN_SIGNER_TYPE, multisig_threshold],
    )
    exec_txn = await account.execute_v1(
    ...
    assert (
        ...
        is True
    ), "no webauthn secp256r1 signer added event emitted"

    await is_account_signer(account, poseidon_hash_many(webauthn_secp256r1_pubk))
    ...
    webauthn_secp256r1_signer = create_webauthn_signer(webauthn_secp256r1_keypair[0])
    if multisig_threshold == 0:
        ...
        if hws_secp256r1_signer is None:
            ...
            # stark + hws should fail
            stark_hws_multisg = create_multisig_signer(
                stark_signer, hws_secp256r1_signer
            )
            await assert_execute_fails_with_signer(
                account,
                balanceof_call,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (generate_secp256r1_keypair(), generate_secp256r1_keypair(), 0, ETHER, 0),
        (
...
async def test_remove_secp256r1_signer(
    ...
    remove_secp256r1_call = Call(
        ...
        calldata=[poseidon_hash_many(secp256r1_pubk), SECP256R1_SIGNER_TYPE, 0],
    )
    ...
    if multisig_threshold == 0:
        ...
        account.signer = (
            create_multisig_signer(stark_signer, secp256r1_signer)
            if webauthn_secp256r1_signer is None
            else create_multisig_signer(secp256r1_signer, webauthn_secp256r1_signer)
        )
    ...
    assert (
        ...
        is True
    ), "expected secp256r1 removed event emitted"
    assert (
        txn_receipt_contains_event(
            res,
            [
        ...
        is False
    ), "did not expect a deferred req cancellation event"
    ...
    assert txn_receipt_contains_event(
        ...
        withdrawal_limit_low > 0 and webauthn_secp256r1_signer is None
    ), "withdrawal limit low not removed"
    ...
    if webauthn_secp256r1_signer is None:
        assert account_signers == [
            1,
            KeyPair.from_private_key(stark_privk).public_key,
            0,
            0,
    ...
    account.signer = (
        stark_signer if webauthn_secp256r1_signer is None else webauthn_secp256r1_signer
    )
    balanceof_call = Call(
        to_addr=int(FEE_CONTRACT_ADDRESS, 16),
        selector=get_selector_from_name("balanceOf"),
    ...
    res = await devnet_client.wait_for_tx(exec_txn.transaction_hash)
    assert (
        res.execution_status == TransactionExecutionStatus.SUCCEEDED
    ), "failed executing with stark signer"
    ...
    if webauthn_secp256r1_signer is None:
        ...
        assert (
            res.execution_status == TransactionExecutionStatus.SUCCEEDED
        ), "failed executing with legacy stark signer"
...
async def test_remove_webauthn_signer(
    ...
    remove_webauthn_secp256r1_call = Call(
        to_addr=account.address,
        selector=get_selector_from_name("remove_secp256r1_signer"),
        calldata=[poseidon_hash_many(webauthn_secp256r1_pubk), WEBAUTHN_SIGNER_TYPE, 0],
    )
    ...
    if multisig_threshold == 0:
        ...
        account.signer = (
            create_multisig_signer(stark_signer, webauthn_secp256r1_signer)
            if hws_secp256r1_signer is None
            else create_multisig_signer(hws_secp256r1_signer, webauthn_secp256r1_signer)
        )
    ...
    if hws_secp256r1_keypair is None:
        ...
        assert account_signers == [
            ...
            KeyPair.from_private_key(stark_privk).public_key,
            1,
            poseidon_hash_many(hws_secp256r1_pubk),
            0,
        ], "No additional signers expected"

    # verify existing signer can sign
    account.signer = (
        stark_signer if hws_secp256r1_signer is None else hws_secp256r1_signer
    )
    balanceof_call = Call(
        to_addr=int(FEE_CONTRACT_ADDRESS, 16),
        selector=get_selector_from_name("balanceOf"),
...
async def test_change_secp256r1_signer(
    ...
    stark_signer = create_stark_signer(stark_privk)
    curr_multisig_signer = create_multisig_signer(stark_signer, curr_secp256r1_signer)
    new_secp256r1_signer = create_secp256r1_signer(new_secp256r1_keypair[0])
    new_multisig_signer = create_multisig_signer(stark_signer, new_secp256r1_signer)
    if multisig_threshold == 0:
        account.signer = curr_secp256r1_signer
    elif multisig_threshold == 2:
    ...
    assert (
        ...
        is True
    ), "no secp256r1 signer removed event emitted"
    ...
    await assert_execute_fails_with_signer(
        account, balanceof_call, stark_signer, "INVALID_SIG"
    )
    if multisig_threshold == 0:
        await assert_execute_fails_with_signer(
            account, balanceof_call, curr_secp256r1_signer, "INVALID_SIG"
        )
        account.signer = new_secp256r1_signer
    elif multisig_threshold == 2:
        await assert_execute_fails_with_signer(
            account, balanceof_call, curr_multisig_signer, "INVALID_SIG"
        )
...
async def test_change_weauthn_signer(
    ...
    stark_signer = create_stark_signer(stark_privk)
    curr_multisig_signer = create_multisig_signer(stark_signer, curr_webauthn_signer)
    new_weauthn_signer = create_webauthn_signer(new_secp256r1_keypair[0])
    new_multisig_signer = create_multisig_signer(stark_signer, new_weauthn_signer)
    if multisig_threshold == 0:
        account.signer = curr_webauthn_signer
    elif multisig_threshold == 2:
        account.signer = curr_multisig_signer
    ...
    if multisig_threshold == 0:
        await assert_execute_fails_with_signer(
            account, balanceof_call, curr_webauthn_signer, "INVALID_SIG"
        )
...
async def test_add_secp256r1_signer_m_of_n(
    ...
    for i in range(signers_num - 1):
        ...
        signer = (
            ...
            if signers_type == SECP256R1_SIGNER_TYPE
            else create_webauthn_signer(secp256r1_keypair[0])
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2, [SECP256R1_SIGNER_TYPE, WEBAUTHN_SIGNER_TYPE, WEBAUTHN_SIGNER_TYPE]),
        (2, [WEBAUTHN_SIGNER_TYPE, SECP256R1_SIGNER_TYPE, SECP256R1_SIGNER_TYPE]),
        (3, [SECP256R1_SIGNER_TYPE, WEBAUTHN_SIGNER_TYPE, WEBAUTHN_SIGNER_TYPE]),
        (3, [WEBAUTHN_SIGNER_TYPE, SECP256R1_SIGNER_TYPE, SECP256R1_SIGNER_TYPE]),
        (
            3,
            [
                SECP256R1_SIGNER_TYPE,
...
async def test_secp256r1_and_webauthn_signers_m_of_n(
    ...
    signers = []
    await add_signers(
        signers_types, multisig_threshold, account, devnet_client, signers
    )
    ...
    if multisig_threshold < len(signers):
        # sig num > threshold
        account.signer = create_multisig_signers(signers[: multisig_threshold + 1])
        await execute_calls(account, balanceof_call, max_fee=10**16)
